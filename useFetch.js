...
export function useFetch(url, type = 'json', options = {}) {
  let array = [];
  array.___toFetch___ =
    { fetchSymbol, url, type, options, _stack: stack() };
  return array;
}
...
export async function goFetch(state, setState, toFetch) {
...
async function _fetch(url, options, type, _stack) {
  debugLogFetch(_stack, url);
  return await (await fetch(url, options))[type]();
}
